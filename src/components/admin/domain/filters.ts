...
export function filterAdminRecords<T extends FilterableAdminRecord>(
  records: T[],
  filters: AdminFilterState,
): T[] {
  const compact = (value: string) =>
    normalizeAdminSearch(value).replace(/[^a-z0-9]/g, "");
  ...
  return records.filter((record) => {
    ...
    const matchesLocation =
      filters.sedeId === "todas" || record.sedeId === filters.sedeId;
