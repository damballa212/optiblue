...
export function validateProductForm(
  ...
  if (value.precio.trim() === "" || !Number.isFinite(price) || price < 0)
    errors.precio = "Debe ser 0 o mayor.";
  const stock = Number(value.stock);
  if (value.stock.trim() === "" || !Number.isInteger(stock) || stock < 0)
    errors.stock = "Debe ser un entero de 0 o mayor.";
  if (value.imagenUrl.trim() && !isHttpUrl(value.imagenUrl.trim()))
    errors.imagenUrl = "Usa una URL HTTP o HTTPS valida.";
  return errors;
}
...
export function validateCategoryForm(
  ...
  if (value.orden.trim() === "" || !Number.isInteger(order) || order < 0)
    errors.orden = "Debe ser un entero de 0 o mayor.";
  return errors;
}
...
export function validateLocationForm(
  ...
  if (!value.maps.trim() || !isHttpUrl(value.maps.trim()))
    errors.maps = "Usa una URL HTTP o HTTPS valida.";
  return errors;
}
