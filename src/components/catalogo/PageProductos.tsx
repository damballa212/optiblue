...
export function PageProductos() {
  ...
  return (
    <div style={section}>
      ...
      {loading && <StatusBlock kind="loading" title="Cargando catálogo" message="Estamos consultando productos y categorías disponibles." />}
      {!loading && error && <StatusBlock kind="error" title="No pudimos cargar el catálogo" message="Revisa la conexión o intenta de nuevo en unos minutos." />}
      {!loading && !error && productos.length === 0 && <StatusBlock kind="empty" title="Catálogo pendiente" message="Todavía no hay productos publicados. Puedes escribir por WhatsApp para consultar disponibilidad en tienda." />}
      {!loading && !error && productos.length > 0 && filtered.length === 0 && (
